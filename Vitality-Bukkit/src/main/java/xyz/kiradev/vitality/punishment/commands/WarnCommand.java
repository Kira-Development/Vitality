...
public class WarnCommand {
    ...
    public WarnCommand(Vitality plugin) {
        this.plugin = plugin;
    }
        @Command(label = "warn", permission = "core.command.warn", appendStrings = true)
        public void warnCommand(CommandSender sender, @Parameter(name = "target") Player target, @Parameter(name = "reason") String reason) {
            if(!(sender instanceof Player)) {
        new Punishment(UUID.randomUUID(), UUID.randomUUID(), target.getUniqueId(), reason, PunishmentType.WARN, Long.MAX_VALUE, plugin.getConfigFile().getString("global.console-format"));
            } else {
                Player player = (Player) sender;
                Profile profile = plugin.getApi().getApi().getProfileManager().getProfileByID(player.getUniqueId());
                new Punishment(UUID.randomUUID(), player.getUniqueId(), target.getUniqueId(), reason, PunishmentType.WARN, Long.MAX_VALUE, profile.getCurrentServer());
            }
        }
    }
