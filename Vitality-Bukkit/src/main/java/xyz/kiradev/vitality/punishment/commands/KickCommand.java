...
public class KickCommand {
    ...
    public KickCommand(Vitality plugin) {
        this.plugin = plugin;
    }

    @Command(label = "kick", permission = "core.command.kick", appendStrings = true)
    public void blacklistCommand(CommandSender sender, @Parameter(name = "player") Player target, @Parameter(name = "reason") String reason) {
        if(!(sender instanceof Player)) {
            Profile punishedProfile = plugin.getApi().getApi().getProfileManager().getProfileByID(target.getUniqueId());
            punishedProfile.addPunishment(new Punishment(UUID.randomUUID(), UUID.randomUUID(), target.getUniqueId(), reason, PunishmentType.KICK, -1, plugin.getConfigFile().getString("global.console-format")));
        } else {
            Player player = (Player) sender;
            Profile issuerProfile = plugin.getApi().getApi().getProfileManager().getProfileByID(player.getUniqueId());
            Profile punishedProfile = plugin.getApi().getApi().getProfileManager().getProfileByID(target.getUniqueId());
            punishedProfile.addPunishment(new Punishment(UUID.randomUUID(), player.getUniqueId(), target.getUniqueId(), reason, PunishmentType.KICK, -1, issuerProfile.getCurrentServer()));
        }
    }
}